...
# except Exception as e:
#     log_message(f"Error membaca config.json: {e}", "error")
#     send_to_frontend("❌ Gagal membaca konfigurasi.")
    

# cookie = config.get("fix_cookies", {})
# user_agent = config.get("user_agent", "")
# spc_cds = cookie.get("SPC_CDS", "")

# if not cookie:
#     log_message("Cookies kosong atau tidak ditemukan!", "error")
#     send_to_frontend("❌ Cookies kosong atau tidak ditemukan! Harap periksa config.json")
...
#     log_message(f"package_param_list response polos: {response_package_param_list}")
#     log_message(f"package_param_list response json: {package_param_list}")

#     index_list = package_param_list["data"]["index_list"]
#     total_order = package_param_list["data"]["pagination"].get("total", 0)
...
#     log_message(f"formatted_data_package_param_list: {formatted_data_package_param_list}")
...
#     log_message(f"order_list response polos {response_order_list}")
#     log_message(f"order_list response json {order_list}")
...
#                 }
#                 for item_group in order["package_card"]["item_info_group"].get("item_info_list", [])
#                 for item in item_group.get("item_list", [])
#             ],
#         }
#         for order in order_list["data"]["card_list"]
#     ]

#     order_list_path = os.path.join(base_path, "sys", "order_list.json")

#     try:
#         with open(order_list_path, "w") as json_file:
#             json.dump(formatted_data_order_list, json_file, indent=2)
        
#         log_message(f"formatted_data_order_list: {formatted_data_order_list}")
...
#     except Exception as e:
#         error_message = f"Gagal menyimpan order_list: {str(e)}"
#         log_message(error_message)
#         send_to_frontend(f"❌ {error_message}")
...
#     log_message(f"Error di get_data.py: {str(e)}\n{error_trace}", "error")
